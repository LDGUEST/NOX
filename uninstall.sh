...
NOX_CODEX_SKILLS="architect armor audit brainstorm changelog cicd commit context-engineer deploy diagnose env full-phase guardrails handoff help-forge iterate migrate perf prompt questions quick-phase refactor review scan security skill-create start syncagents tdd update uxtest api schema landing doc swot monitorlive explain"
...
if [ "$REMOVE_HOOKS" = true ]; then
  if [ -d "$HOOKS_DEST" ]; then
    hook_count=0
    # Remove all Nox hooks (.sh and .js)
    for hook in destructive-guard sync-guard secret-scanner debug-reminder build-tracker cost-alert notify-complete notify-timer-start commit-lint test-regression-guard file-size-guard todo-tracker drift-detector auto-context compact-saver session-logger agent-tracker prompt-guard memory-auto-save nox-metrics nox-parse lib-json session-cost-tracker; do
      for ext in sh js; do
        if [ -f "$HOOKS_DEST/$hook.$ext" ]; then
          rm -f "$HOOKS_DEST/$hook.$ext"
          hook_count=$((hook_count + 1))
        fi
      done
    done
    # Remove JS-only hooks
    for hook in context-monitor statusline-unified gsd-context-monitor; do
      if [ -f "$HOOKS_DEST/$hook.js" ]; then
        rm -f "$HOOKS_DEST/$hook.js"
        hook_count=$((hook_count + 1))
      fi
    done
