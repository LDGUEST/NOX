...
for arg in "$@"; do
  case "$arg" in
    ...
    --with-registry) INSTALL_REGISTRY=true ;;
    ...
    --help|-h)
      ...
      echo "  --with-registry Install extended/niche skills (api, schema, landing, doc, swot, monitorlive, explain)"
...
# ── Registry Skills (extended/niche, opt-in) ──────────────────
REGISTRY_SRC="$SCRIPT_DIR/registry"
if [ "$INSTALL_REGISTRY" = true ] && [ -d "$REGISTRY_SRC" ]; then
  echo "Installing Nox registry (extended) skills..."

  if [ "$INSTALL_CLAUDE" = true ] && [ -d "$REGISTRY_SRC/claude" ]; then
    reg_count=0
    for skill in "$REGISTRY_SRC/claude"/*.md; do
      [ -f "$skill" ] || continue
      name="$(basename "$skill")"
      install_file "$skill" "$CLAUDE_DEST/nox/$name"
      reg_count=$((reg_count + 1))
    done
    echo "  -> $reg_count registry skills installed for Claude Code"
  fi

  if [ "$INSTALL_GEMINI" = true ] && [ -d "$REGISTRY_SRC/gemini" ]; then
    reg_count=0
    for skill_dir in "$REGISTRY_SRC/gemini"/*/; do
    ...
    echo "  -> $reg_count registry skills installed for Gemini CLI"
  fi

  if [ "$INSTALL_CODEX" = true ] && [ -d "$REGISTRY_SRC/codex" ]; then
    reg_count=0
    for skill_dir in "$REGISTRY_SRC/codex"/*/; do
      [ -d "$skill_dir" ] || continue
      name="$(basename "$skill_dir")"
      mkdir -p "$CODEX_DEST/$name"
      install_file "$skill_dir/SKILL.md" "$CODEX_DEST/$name/SKILL.md"
    ...
    echo "  -> $reg_count registry skills installed for Codex CLI"
  fi
else
  if [ "$INSTALL_REGISTRY" = true ]; then
    echo "Registry directory not found — skipping extended skills"
  fi
fi
