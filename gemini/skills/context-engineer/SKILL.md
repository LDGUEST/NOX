...
description: Audits and governs all AI context files — health scoring, armor enforcement, bloat detection, cross-project drift, context reset. Use when syncing CLAUDE.md files, diagnosing context issues, onboarding a project, or resetting stale assumptions.
---
...
- `--reset <component>` — context reset: purge all prior assumptions about a component and set new truth
...
─────────────────────────────────────────────────
my-saas-app              72/100  C       [no armor] [stale env vars]
admin-dashboard          88/100  B+      [1 stale entry]
...
### Context Reset (`--reset <component>`)

When invoked with `--reset`, treat the user's latest input as an absolute state update for the named component. Overwrite and purge any conflicting previous context, rules, or architectural decisions regarding that component.

1. **Identify** — List all prior assumptions, decisions, and context about the component
2. **Discard** — Confirm exactly what outdated assumptions are being discarded
3. **Replace** — Accept the new input as the definitive source of truth
4. **Verify** — Ensure no stale references to the old context remain in any context files

Do not stack new information on top of old data — this is a clean replacement. The user must see exactly what was purged so context is perfectly clean before proceeding.
