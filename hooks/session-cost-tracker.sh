...
MACHINE="${NOX_MACHINE_NAME:-unknown}"
if [ "$MACHINE" = "unknown" ]; then
    # Auto-detect from hostname — customize these patterns for your setup
    MACHINE="$(hostname 2>/dev/null | tr '[:upper:]' '[:lower:]' | head -c 20)"
fi
...
# ── Central DB (optional) ──
# Set NOX_CENTRAL_DB_PATH and NOX_CENTRAL_SSH to aggregate metrics across machines.
# If the central DB is unreachable, writes to local fallback DB.
CENTRAL_DB_PATH="${NOX_CENTRAL_DB_PATH:-}"
CENTRAL_SSH="${NOX_CENTRAL_SSH:-}"
...
if [ -n "$CENTRAL_SSH" ] && [ -n "$CENTRAL_DB_PATH" ]; then
    # Central DB configured — try SSH insert (hard 5s timeout)
    if timeout 5 ssh -o ConnectTimeout=2 -o BatchMode=yes -o ServerAliveInterval=2 -o ServerAliveCountMax=1 "$CENTRAL_SSH" \
        "sqlite3 '$CENTRAL_DB_PATH' \"$CREATE_SQL\" && sqlite3 '$CENTRAL_DB_PATH' \"$INSERT_SQL\"" 2>/dev/null; then
        true  # Success — logged to central DB
    elif [ "$HAS_SQLITE3" = true ]; then
        # Central DB unreachable — write locally for later merge
